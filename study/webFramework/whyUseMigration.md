...
### 프로덕션 환경에서는 롤백 보다는 마이그레이션을 사용하자.
- 마이그레이션은 일반적으로 기능의 개선 및 추가를 위해 정의되는 경우가 많으므로 마이그레이션이 이뤄지면 변경된 스키마에 맞게 데이터가 추가되는 경우가 대부분이다. 롤백을 하게 되면 변경된 스키마에 맞게 새롭게 생성된 데이터들이 소실되는 경우가 발생할 수 있다.
- 마이그레이션의 롤백을 정의할 때는 스키마의 변경을 되돌리는 것에 초점을 맞출 뿐 새롭게 생성된 데이터의 손실에 대해서는 보통 생각하지 않고 롤백 코드를 정의한다. 따라서 프로덕션 환경에서 스키마의 변경으로 무언가가 잘못되었을 때는 새롭게 마이그레이션을 만들어서 데이터의 손실을 최소화 할 수 있는 스키마 변경 방안을 마련하는 편이 좋다.
- 물론 아주 초기 단계에 문제가 발견되는 경우와 같이 데이터가 손실 되어도 상관이 없는 경우에는 롤백을 허용해도 된다. 하지만 일반적으로 롤백은 자동화되어 있지 않고 수동으로 적용해야 하는 경우가 많고 수동으로 롤백을 하다가 롤백을 해서는 안되는 대상도 롤백을 실행할 가능성이 있게 되어 데이터를 손상시킬 가능성이 있다. 이런 실수를 미연에 방지하기 위해서 롤백을 최대한 배제하고 크리티컬하지 않은 문제라면 새로운 마이그레이션을 정의하여 로컬과 검증환경에서 스키마 변경 사항을 테스트를 한 후에, 새로 만들어진 마이그레이션을 프로덕션 환경에 적용하여 이전의 마이그레이션으로 발생한 문제를 해결하는 편이 좋다.